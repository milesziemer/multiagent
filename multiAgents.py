...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        #Get ghost position and pacman position
        ...
        totalGhostDistance = 0 #initialize the total distance from ghosts

        for ghost in ghostPosition:
            ghostDistance = manhattanDistance(pacmanPosition,ghost)
            if ghostDistance > 0:
                ghostDistance = 1/(ghostDistance**2)
            else:
                ghostDistance = 999
            totalGhostDistance += ghostDistance

        #get distance from food
        closestFood = 999 #initialize distance from closest food
        foodDistance = 999 #initialize an individual food distance
        for food in foodPositions:
            x = food[0]
            y = food[1]
            if newFood[x][y]:
                foodDistance = (((pacmanPosition[0] - x)**2 + (pacmanPosition[1] - y)**2))
                if foodDistance < closestFood:
                    closestFood = foodDistance
        if closestFood > 0:
            closestFood = 1/closestFood

        return successorGameState.getScore() - 2*totalGhostDistance + closestFood
...
def scoreEvaluationFunction(currentGameState):
    ...
    This betterEvaluationFunctionluation function is meant for use with adversarial search agents
    (not reflex agents).
    """
